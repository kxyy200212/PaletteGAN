...
Please place the required checkpoint files in the `checkpoints/` folder before training, inference, or testing.
...
The VGG16 checkpoint is used for perceptual, style, and perceptual color losses. The PaletteGAN checkpoint can be used for inference and testing.

If the trained PaletteGAN checkpoint is not available, users can reproduce the model by running the training script with the fixed data splits and configuration provided in this repository.
